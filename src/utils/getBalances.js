...
export default function BalancesWidget() {
  ...
  useEffect(() => {
    ...
    fetchBalances(); // Pirma karta uzkraunam
  ...
  return (
    <div style={{ color: "white", textAlign: "center", marginTop: "20px" }}>
      <h2 style={{ marginBottom: "20px" }}>Your Balances</h2>
      {Object.entries(balances).map(([network, info]) => (
        <div key={network} style={{ marginBottom: "12px" }}>
          <strong>{info.symbol}:</strong>{" "}
          {info.balance !== null ? `${parseFloat(info.balance).toFixed(4)} ${info.symbol}` : "N/A"}
        </div>
      ))}
    </div>
  );
